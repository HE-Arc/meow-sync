...
class OAuthLoginView(APIView):
    ...
    def get(self, request, provider: str) -> Response:
        if provider not in MUSIC_PROVIDERS.keys():
            raise serializers.ValidationError(f'This field must be an valid provider. providers: {" ".join(MUSIC_PROVIDERS.keys())}')

        oauth_state = OAuthState(user=request.user, provider=provider, state=random_str_alphanum(250))
        state.save()

        return Response(
            {'provider': provider, 'state': oauth_state.state, 'login_url': SpotifyApi.login_url(oauth_state.state)},
...
class OAuthCallbackView(APIView):    
    def get(self, request, provider: str) -> Response:
        ...
        print(f'OAuth  callback with state: {state}, code: {code}, provider: {provider}')
        ...
        try:
            ...
            provider_api_instance = provider_api_class(access_token=provider_tokens.access_token)
            provider_user_response: ApiResponse[ApiUser] = provider_api_instance.get_current_user()

            if not provider_user_response.success:
                print(f'Could not get user data for provider {oauth_state.provider} - {provider_user_response.message}')
                return Response(
            ...
            if not oauth_state.user:
                ...
                if provider_user_response.data.name:
                    username = (provider_user_response.data.name[:240]
                        .lower()
                        .replace('  ', ' ')         # replace double spaces
                        .replace(' ', '_')          # replace spaces with '_'
                        [: 245]                     # truncate string
                        + random_str_alphanum(4))   # some randomness for users with same name

                user = User.objects.create(username=username, password=random_str_alphanum(255))
                user.save()
            ...
            print(f'More than one OAuthState with the same state: {state}. Something went horribly wrong.')
...
class OAuthDisconnectView(APIView):
    ...
    def delete(self, request, provider: str):
        ...
        try:
            ...
            print(f'More than one OAuthConnection with the same provider: {provider}. Something went horribly wrong.')
